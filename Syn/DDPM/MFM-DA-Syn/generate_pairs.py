...
with torch.no_grad():
    for batch_idx,imgs in enumerate(real_dataloader):
        ...
        # save_image(torch.cat([imgs,mid_imgages[imgs.size(0)*2:imgs.size(0)*3]],dim=0),os.path.join(opts.save_path,"A-%d.png"%batch_idx),nrow=imgs.size(0))
        # save_image(mid_imgages, os.path.join(opts.save_path, "%d-mid.png" % batch_idx),nrow=imgs.size(0))
        save_image(mid_imgages[imgs.size(0)*2:imgs.size(0)*3], os.path.join(opts.save_path, f"{batch_idx}_{tag}.jpg"),nrow=imgs.size(0))
        save_image(imgs, os.path.join(opts.save_path, f"{batch_idx}.jpg"),
                   nrow=imgs.size(0))
