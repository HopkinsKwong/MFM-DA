...
def build_Prostate(images_path_list, labels_path_list,image_set_tag):

    image_paths, label_paths = sorted(images_path_list), sorted(labels_path_list)
    path_dicts = [
    ...
    if image_set_tag == "train":
        slice_transform = slice_transform_train
    elif image_set_tag == "val":
        slice_transform = slice_transform_valid

    dataset = CacheDataset(
        data=path_dicts, transform=volume_transform, cache_rate=1.0, num_workers=16
    )
