...
class Reins(nn.Module):
    ...
    def create_model(self):
        ...
        self.cnn_scale = nn.Parameter(torch.tensor(0.001))
        self.cnn_mlp_token2feat = nn.Linear(self.embed_dims, self.embed_dims)
        self.cnn_mlp_delta_f = nn.Linear(self.embed_dims, self.embed_dims)
        ...
        nn.init.kaiming_uniform_(self.cnn_mlp_delta_f.weight, a=math.sqrt(5))
        nn.init.kaiming_uniform_(self.cnn_mlp_token2feat.weight, a=math.sqrt(5))
    ...
    def forward(
        self, feats: Tensor, cnn_feats,layer: int, batch_first=False, has_cls_token=True
    ) -> Tensor:
        if batch_first:
        ...
        if cnn_feats is not None:
            if batch_first:
                cnn_feats = cnn_feats.permute(1, 0, 2)
            delta_cnn_feats = self.forward_delta_cnn_feat(
                feats,
                cnn_feats
            )
            delta_cnn_feats = delta_cnn_feats * self.cnn_scale
            feats = feats + delta_feat + delta_cnn_feats
        else:
    ...
    def forward_delta_cnn_feat(self, feats: Tensor,cnn_feats : Tensor) -> Tensor:
        # print(f"feats.shape - {feats.shape}")
        # print(f"cnn_feats.shape - {cnn_feats.shape}")
        attn = torch.einsum("nbc,nmc->nbm", feats, cnn_feats)
        if self.use_softmax:
            attn = attn * (self.embed_dims ** -0.5)
            attn = F.softmax(attn, dim=-1)
        cnn_delta_f = torch.einsum(
            "nbm,nmc->nbc",
            attn[:, :, :],
            self.cnn_mlp_token2feat(cnn_feats[:, :]),
        )
        cnn_delta_f = self.cnn_mlp_delta_f(cnn_delta_f + feats)
        return cnn_delta_f
